...
def update_current_user_profile(
    ...
    if data.settings and isinstance(data.settings, dict):
        for field in settings_fields:
            if field in data.settings and data.settings[field] is not None:
                setattr(settings_obj, field, data.settings[field])
                settings_updated = True
    ...
    if data.preferred_language is not None and data.language is None:
        if not data.settings or "language" not in data.settings:
...
def delete_current_user_profile(
    ...
    # 1. Delete associated system records
    ...
    # 3. Delete AI conversations, reports, and analysis runs
    ...
    if settings.SUPABASE_URL and settings.SUPABASE_SERVICE_ROLE_KEY:
        try:
            ...
            logger.warning(
                "Could not delete auth user %s from Supabase Admin: %s", auth_user_id, exc
            )
